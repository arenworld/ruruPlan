...
import time
from python.newQuestion import run_id1


content2 = sys.argv[1] if len(sys.argv) > 1 else ""
...
thread_message = client.beta.threads.messages.create(
    thread_id,
    role="user",
    content=content2 + "부분만 다시 추천해줘. 다른건 안보내줘도 돼.",
)
...
while True:
    ...
    if run.status == "completed":
        ...
        time.sleep(3)
    # print("status :" + run.status)      # status='completed'라고 나온다. 실패하면 failed, 진행중일때는 in_progress가 나온다.
